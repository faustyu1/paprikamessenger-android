package ru.faustyu.paprika.util

import java.math.BigInteger
import java.security.SecureRandom

object CryptoManager {
    // Same Prime as Backend (Simplified for prototype)
    private val P = BigInteger("", 16)
    private val G = BigInteger("2")

    private var privateKey: BigInteger? = null
    var publicKey: BigInteger? = null
        private set

    fun generateKeys() {
        val random = SecureRandom()
        privateKey = BigInteger(2048, random).mod(P)
        publicKey = G.modPow(privateKey, P)
    }

    fun computeSharedSecret(serverPublicKeyStr: String): BigInteger {
        val serverPublicKey = BigInteger(serverPublicKeyStr, 16)
        if (privateKey == null) throw IllegalStateException("Keys not generated")
        return serverPublicKey.modPow(privateKey, P)
    }

    // TODO: AES Encryption using the shared secret
    // fun encrypt(msg: String, secret: BigInteger): String { ... }
}
